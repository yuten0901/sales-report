...
- **CSVインジェクション対策**(`=` `+` `-` `@`で始まる値の無害化)、**原子的書き込み**(処理中断時に壊れた出力を残さない)、**冪等性**(同一入力での再実行結果がバイト同一)
- **プロパティベーステスト**(Hypothesis): 総合計が店舗別/商品別/日別集計の合計と必ず一致する、行の入力順序を変えても結果が不変、等
- **ゴールデンテスト**で出力フォーマットの回帰を検知
- **性能テスト**: 10万行を約7秒・メモリ500MB未満で処理(通常実行から分離、`pytest -m slow`で実行)
...
- **金額計算は`Decimal`のみで行う**(floatを一切使わない)。丸め誤差を出さないための設計上の制約
