...
## FIX-11: conftest.pyの虚偽docstring・未使用のfreezegun依存

- **発見日**: 2026-07-26
- **検出手段**: 公開前レビュー（別コンテキスト）。「`tests/conftest.py`のdocstringは『決定性の担保(乱数シード・時刻固定)もここで一括管理する』とあるが、実際には何もしていない。`freezegun`も`pyproject.toml`にあるだけで未使用」と指摘。
- **再現手順**: `conftest.py`のdocstringを読んだ後、ファイル内で`random.seed`や`freezegun`の使用箇所を検索する。
- **期待**: docstringに書かれた内容が実装されているか、実装が無いならdocstringにその主張を書かない。
- **実際**: `conftest.py`には乱数シードの固定も時刻フリーズの仕組みも一切存在せず、`freezegun`はどのソースコードからも一度もimportされていなかった。
- **原因分析**: 実装時に「将来こういう機能を足すかもしれない」という想定でdocstringと依存関係だけを先に用意し、実際に必要になった時点で実装するつもりが、実装せずに放置されていた。CLAUDE.mdの「将来の仮定的要件のために設計しない」という原則に反する先回りが、結果的に嘘のドキュメントと未使用依存を生んだ。
...
## FIX-12〜15: CIワークフローの修正（Codex指摘中心）

- **FIX-12（mutation.ymlの初回赤リスク）**: `mutmut results --all`はmutmut 3系ではブール値を伴う形式（例: `--all true`）である可能性が高く、単純なフラグとして呼ぶとCLIエラーになり得るとCodexが指摘。**修正**: `mutmut results --all true` → 失敗時 `--all` → 失敗時 `mutmut results`（no-args）の順に試すフォールバックに変更。あわせて、スコア計算が`killed`/`survived`のみを見て`timeout`/`suspicious`/`no tests`/`skipped`等を分母から除外し過大評価していた点も修正（`Counter`で全ステータスを集計し分母に含める）。⚠️ このワークフローはWindows非対応のためローカルで一度も実行できておらず、`mutmut results`の正確なCLI引数仕様は**初回CI実行まで確定できない**。ゲート（80%閾値）は`continue-on-error: true`のまま「試験運用中」と明記し、初回実行結果を見てから正式化する。
- **FIX-13（pr-summaryがテスト失敗時にskipされる）**: `needs: test`が失敗すると既定でこのジョブ自体がskipされ、**一番結果を見たい失敗時にコメントが出ない**という逆転現象があった（Codex指摘）。**修正**: `if: github.event_name == 'pull_request'` を `if: ${{ !cancelled() && github.event_name == 'pull_request' }}` に変更し、testが失敗してもキャンセルされない限り実行されるようにした。
...
## DEF-017: 正規化後に同名になる重複ヘッダを黙って後勝ちで採用していた（Codex Medium#3）

- **発見日**: 2026-07-27
- **検出手段**: Codexによるリポジトリ全体の再レビュー
- **再現手順**: ヘッダに`date,Date,store,product,quantity,unit_price`のように、正規化（strip+casefold）後に同名となる列を含むCSVを読み込む。
- **期待**: どちらの値が採用されたか不明瞭な状態を許容せず、明示的にエラーとして拒否する。
- **実際**: `{name.strip().casefold(): name for name in reader.fieldnames}`という辞書内包表記は、キーが重複すると**後の列が前の列を黙って上書き**する。利用者はどちらの列の値が使われたか知る術がなかった。
- **原因分析**: ヘッダ正規化（A-1設計裁定）を実装した際、正規化後の一意性を確認する処理が漏れていた。
- **修正**: `load_file`で正規化後の列名の重複を検出し、重複があれば`FileError`（理由に重複した列名を含む）にするよう変更。
- **追加した回帰テスト**: `tests/test_loader.py::test_load_file_duplicate_header_after_normalization_becomes_file_error`（大文字小文字違いの重複）・`test_load_file_duplicate_header_via_whitespace_variant_becomes_file_error`（前後空白違いの重複）。赤→緑確認済み（修正前のコードでは両方失敗することを確認）。
- **教訓**: 「複数の入力表記を1つに正規化する」機能を作るときは、正規化後の一意性もセットで検証しないと、異なる入力が同じ出力に潰れる際の情報損失（今回はどちらが採用されたか）を見落とす。

## DEF-018: 文字コード「フォールバック」を「判定」と称し、UTF-16(BOM付き)が対象外だった（Codex High#2）

- **発見日**: 2026-07-27
- **検出手段**: Codexによるリポジトリ全体の再レビュー
- **再現手順**: (a) UTF-16(BOM付き)のCSVを読み込む、(b) 全エンコーディングで失敗した際のエラーメッセージを確認する。
- **期待**: (a) BOMという明確な手がかりがあるエンコーディングは検出して読めること。(b) エラーメッセージが実態(ベストエフォートのフォールバック)を正確に表現すること。
- **実際**: (a) UTF-16は試行対象になく、BOM付きでも文字コードエラーになっていた。(b) 「文字コードを判定できませんでした」という文言が、実際にはやっていない「判定(detection)」を名乗っていた。加えてcp932は受理範囲が非常に広く、EUC-JP等の他エンコーディングを例外なく誤読し得る構造的な限界がありながら、テストも文書もその限界に触れていなかった。
- **原因分析**: 「UTF-8→Shift-JISの順に試す」という設計だけを行い、それ以外のエンコーディングの扱い（対応するか、しないなら理由）を明文化していなかった。
- **修正**: ファイル先頭2バイトでUTF-16 BOM(LE/BE)を検出できた場合のみ`utf-16`を試行リストに追加(BOM無しUTF-16は判別が原理的に難しいため対象外のまま)。エラーメッセージを「対応する文字コード(UTF-8/Shift-JIS)でデコードできませんでした」に修正し、docstringも「判定」ではなく「フォールバック」と明記。
- **追加した回帰テスト**: `tests/test_loader.py::test_load_file_utf16_with_bom_is_accepted`（BOM付きUTF-16を読める）・`test_load_file_cp932_silently_misreads_other_encodings_known_limitation`（cp932の誤読という**既知の限界そのもの**を記録するテスト。クラッシュしないことではなく、意味的に誤った内容がサイレントに読み込まれる挙動自体を検証）。
- **教訓**: 「検出」と「フォールバック」は違う。前者を名乗るなら精度に責任を持つ必要があるが、後者なら限界を正直に明記すれば十分。文言ひとつが読み手（採用担当者含む）の期待値を歪める。

## DEF-019: 列数がヘッダより多い行の余剰値を無条件に無視していた（Codex High#11）

- **発見日**: 2026-07-27
- **検出手段**: Codexによるリポジトリ全体の再レビュー
- **再現手順**: `2026-07-01,渋谷店,商品A,3,1200,備考,余分`のように、ヘッダ(5列)より多い値を持つ行を読み込む。
- **期待**: 区切り文字の混入・引用符の破損による列ずれの可能性を無視せず、何らかの形で利用者に伝える。
- **実際**: `csv.DictReader`の既定動作で、余剰値は`restkey=None`のキーに黙って吸収され、必須列さえ揃っていれば正常な行として処理されていた（テスト名も「安全に無視される」という誤った前提で書かれていた）。
- **原因分析**: 「堅牢性」と「データ破損の隠蔽」を区別できていなかった。列数が合わないこと自体が、入力データに何らかの異常がある強いシグナルである。
- **修正**: `csv.DictReader(..., restkey="__extra__")`とし、余剰値に**非空**のものが1つでもあれば行エラーにする。行末の区切り文字による空の余剰値（例: 末尾`,`）のみは無害として許容し、誤検知を避ける。
- **追加した回帰テスト**: `tests/test_loader.py::test_load_file_excess_non_empty_columns_become_row_error`（非空の余剰値→行エラー）・`test_load_file_trailing_empty_extra_column_is_tolerated`（末尾の空の余剰値→正常処理）。赤→緑確認済み。
- **教訓**: 「クラッシュしない」と「データを正しく扱えている」は別物（DEF-009の教訓と同型）。今回は「無視して処理を続ける」という一見親切な設計が、実際にはデータ破損を隠蔽していた。

## DEF-020: 原子的置換が既存ファイルのPOSIXパーミッションを破壊する（Codex High#4）

- **発見日**: 2026-07-27
- **検出手段**: Codexによるリポジトリ全体の再レビュー
- **再現手順**（Linux想定）: 権限0644の既存レポートファイルに対し、`sales-report`を再実行して上書きする。
- **期待**: 既存ファイルの権限は、内容の更新によって変わらないこと。
- **実際**: `tempfile.mkstemp()`が作る一時ファイルは既定で0600(所有者のみ読み書き可)。これを`os.replace()`で本番パスへ置換すると、既存ファイルが0644等の権限を持っていても**0600に変わってしまう**。Linuxの共有ディレクトリ・Web公開ディレクトリ・後続ジョブが読めなくなり得る。
- **原因分析**: 原子的書き込み(一時ファイル→`os.replace()`)の実装時、ファイル内容の整合性(中断時に中途半端な内容を残さない)にのみ注意が向いており、パーミッションという「内容以外の属性」が置換によって変わり得ることを見落としていた。**Windows開発機では原理的に検出できない欠陥**(Windowsはmodeビットの意味が限定的)。
...
## FIX2-16: `# nosec B101`の妥当性(Codex再確認・変更なし)

Codex再レビューでも「これらは外部入力のセキュリティ検証ではなく、直前のvalidator結果をmypy向けに絞る内部不変条件であり、妥当」と結論（`src/sales_report/models.py:201-205`）。型付きヘルパー化による可読性向上の余地はあるとの指摘もあったが、挙動を変えないリファクタリングであり今回の修正スコープ(実バグ・テスト品質)に見合わないと判断し、**変更しない**。判断した事実のみ記録する。
