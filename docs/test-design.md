...
| ミューテーションテスト | 手動での意図的なコード変異による、テストの検知力そのものの検証（mutmutはローカル未実行・手動代替）。スコア集計自体は`scripts/mutation_score.py`としてpytestでテスト可能（FIX2-01/14・2026-07-27追加） | `docs/test-report.md` §5、`tests/test_mutation_score.py` |
